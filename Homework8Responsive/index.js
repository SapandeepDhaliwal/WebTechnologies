...
function initialize(req) {
     ...
     var url1= 'https://maps.googleapis.com/maps/api/geocode/json?address='+street+city+state+'&key=00000000000000000000000000000000000';//Add key
    return new Promise(function(resolve, reject) {
    	https.get(url1, (resp) => {
            ...
            resp.on('end', () => {
              ...
              url2 = "https://api.darksky.net/forecast/00000000000000000000000000/"+lat+","+lng;//Add key
              return resolve(url2);
            });
...
app.get("/lat/:latId&long/:longId",function (req,res){
    ...
    var newurl=url2 = "https://api.darksky.net/forecast/000000000000000000000000000000/"+lat+","+lng;//Add key
...
app.get("/lat/:latId&long/:longId&time/:time",function (req,res){
  ...
  var newurl = "https://api.darksky.net/forecast/00000000000000000000000000000/"+lat+","+lng+","+time; //Add key
  secondrequest(newurl,res);
})
...
app.get("/state/:stateId",function (req,res){
  var state =req.params.stateId;
  var newurl = "https://www.googleapis.com/customsearch/v1?q=Seal%20of%20"+state+"&cx=010227509778041328616:obfawgaloha&num=1&searchType=image&key=00000000000000000000000000000000000"; //Add key
  secondrequest(newurl,res);
})
...
app.get("/city/:cityId",function (req,res){
  var city =req.params.cityId;
  var newurl = "https://maps.googleapis.com/maps/api/place/autocomplete/json?input="+city+"&types=(cities)&language=en&key=000000000000000000000000000"; //Add key
